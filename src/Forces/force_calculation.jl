...
function wall_force(x, i, ϵ, σ, b, L, F)
    ...
    for ip = 1:x[b]["n"]
        ...
        FW_x1        = F_lj_2d([x[b]["position"][i, ip, 1] - (-L), 0.0u"nm"], ϵ, σ)[1]
        FW_x2        = F_lj_2d([x[b]["position"][i, ip, 1] - (+L), 0.0u"nm"], ϵ, σ)[1]
        FW_y1        = F_lj_2d([0.0u"nm", x[b]["position"][i, ip, 2] - (-L)], ϵ, σ)[2]
        FW_y2        = F_lj_2d([0.0u"nm", x[b]["position"][i, ip, 2] - (+L)], ϵ, σ)[2]
...
function create_bond(x, viscocity, i, b, F, k, d) #x is an array
